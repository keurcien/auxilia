...
2. **No CI, no CONTRIBUTING, no coverage.** `uv sync && uv run pytest` from a cold clone is green in 4.4s with zero external services — genuinely rare and worth protecting — and nothing runs it. `pyproject.toml` still says `description = "Add your description here"`.
