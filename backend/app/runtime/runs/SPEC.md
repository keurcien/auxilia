...
`Agent.build(thread, db)` to execute — the durable layer wraps `agent.py`, it
does not replace it.
...
## Module layout (`app/runtime/runs/`)

Layered like the rest of the backend (router → service → repository); the router lives in `app/runtime/api/` so this package is the lifecycle alone:
...
| `../api/runs_router.py` | HTTP surface (create / invoke / get / cancel / list / active); live streaming is `../api/protocol_router.py` |
...
`lifecycle` / `input.requested`, see `app/runtime/protocol/emit.py` for the
...
(`POST /threads/{id}/stream/events`, `app/runtime/api/protocol_service.py`) relays
...
- A run carries an opaque `delivery` descriptor (`RunDB.delivery`); `None`
  ...
  not by this module — `app/runtime/runs` never imports `app/integrations`.
