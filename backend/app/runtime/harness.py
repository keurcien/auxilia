...
``tests/runtime/test_harness_parity.py`` builds a graph both ways and asserts
...
- **No ``DeepAgentState``.** 0.7 compiles with a ``DeltaChannel`` on
  ...
  ``api/protocol_service.py``, ``read_run_result``, the thread read endpoint).
...
from app.skills.middleware import skills_index_middleware
...
def _profile(model) -> HarnessProfile:
    ...
    if used:
        msg = (
            ...
            f"{used}, which app/runtime/harness.py does not reproduce. "
            "Port the missing branch from deepagents.graph.create_deep_agent "
            "(and extend tests/runtime/test_harness_parity.py)."
        )
...
def _base_harness_stack(model, backend, profile: HarnessProfile, skills=None) -> list:
    ...
    ``skills`` (the run's `SkillsBackend`, or None) lands where deepagents
    puts `SkillsMiddleware` on a subagent: after the patcher."""
    stack = [
    ...
    if skills is not None:
        stack.append(skills_index_middleware(skills, sandbox=True))
    return stack
...
def harness_middleware(
    *, model, tools: list, backend, subagents=None, skills=None
    ...
    Order matters and mirrors deepagents exactly: skills (when the run has
    ...
    if skills is not None:
        middleware.append(skills_index_middleware(skills, sandbox=True))
    middleware.append(
        FilesystemMiddleware(
            backend=backend,
            custom_tool_descriptions=profile.tool_description_overrides,
