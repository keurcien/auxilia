...
`GET /threads/{id}/state`. This package is the codec — pure translation, no
IO, imports nothing from `runs` — and the HTTP relay lives next to the runs
router in `app/runtime/api/`:
...
- **Relay** (`api/protocol_service.py` / `api/protocol_router.py`): a stream
  session filters the stored
