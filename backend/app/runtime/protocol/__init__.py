...
`GET /threads/{id}/state`. This package is the codec — the server-side grammar and its policies; the
HTTP surface that speaks it lives in `app/runtime/api/`:
...
- **Relay** (`app/runtime/api/service.py` / `router.py`): a stream session
  filters the stored events through the client's sink (`filter.py`) and stamps
  replay cursors from the log entry ids. Nothing is translated on read.
