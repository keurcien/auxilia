import { FileText, TerminalSquare } from "lucide-react";
import type { BundledLanguage, SpecialLanguage } from "shiki";
import { cn } from "@/lib/utils";

/**
 * File-type marks for a skill's files.
 *
 * Skill files are read-only now that scripts only ever arrive from a
 * repository, so the one job left to the tab strip is saying *what* a file
 * is at a glance. Python, TypeScript, JavaScript and Markdown carry their
 * own brand mark (the real logo geometry, flat brand colors); everything
 * else falls back to a monochrome glyph that follows the theme, as does
 * Markdown's, whose mark is black by definition and would vanish on a dark
 * panel.
 */
export type SkillFileKind =
	| "python"
	| "typescript"
	| "javascript"
	| "markdown"
	| "shell"
	| "plain";

const BY_EXTENSION: Record<string, SkillFileKind> = {
	py: "python",
	pyi: "python",
	ts: "typescript",
	tsx: "typescript",
	mts: "typescript",
	cts: "typescript",
	js: "javascript",
	jsx: "javascript",
	mjs: "javascript",
	cjs: "javascript",
	md: "markdown",
	markdown: "markdown",
	sh: "shell",
	bash: "shell",
	zsh: "shell",
};

/** The lowercase extension of a path, or "" when it has none. */
export function fileExtension(path: string): string {
	const name = path.split("/").pop() ?? "";
	const dot = name.lastIndexOf(".");
	return dot > 0 ? name.slice(dot + 1).toLowerCase() : "";
}

export function skillFileKind(path: string): SkillFileKind {
	return BY_EXTENSION[fileExtension(path)] ?? "plain";
}

/**
 * The shiki grammar for a path. Anything without a grammar we are sure of
 * renders as plain text rather than being highlighted as the wrong language.
 */
const BY_LANGUAGE: Record<string, BundledLanguage> = {
	py: "python",
	pyi: "python",
	ts: "typescript",
	mts: "typescript",
	cts: "typescript",
	tsx: "tsx",
	js: "javascript",
	mjs: "javascript",
	cjs: "javascript",
	jsx: "jsx",
	md: "markdown",
	markdown: "markdown",
	json: "json",
	yaml: "yaml",
	yml: "yaml",
	toml: "toml",
	sh: "bash",
	bash: "bash",
	zsh: "bash",
	sql: "sql",
	html: "html",
	css: "css",
	xml: "xml",
};

export function skillFileLanguage(path: string): BundledLanguage | SpecialLanguage {
	return BY_LANGUAGE[fileExtension(path)] ?? "plaintext";
}

interface FileTypeIconProps {
	path: string;
	className?: string;
}

export function FileTypeIcon({ path, className }: FileTypeIconProps) {
	const kind = skillFileKind(path);
	const size = cn("size-3.5 shrink-0", className);

	if (kind === "shell") return <TerminalSquare className={cn(size, "text-petrol")} />;
	if (kind === "plain") return <FileText className={size} />;

	return (
		<svg
			viewBox="0 0 128 128"
			role="img"
			aria-label={kind}
			className={size}
			xmlns="http://www.w3.org/2000/svg"
		>
			{kind === "python" && (
				<>
					<path fill="#3776AB" transform="translate(0 10.26)" d="M63.391 1.988c-4.222.02-8.252.379-11.8 1.007-10.45 1.846-12.346 5.71-12.346 12.837v9.411h24.693v3.137H29.977c-7.176 0-13.46 4.313-15.426 12.521-2.268 9.405-2.368 15.275 0 25.096 1.755 7.311 5.947 12.519 13.124 12.519h8.491V67.234c0-8.151 7.051-15.34 15.426-15.34h24.665c6.866 0 12.346-5.654 12.346-12.548V15.833c0-6.693-5.646-11.72-12.346-12.837-4.244-.706-8.645-1.027-12.866-1.008zM50.037 9.557c2.55 0 4.634 2.117 4.634 4.721 0 2.593-2.083 4.69-4.634 4.69-2.56 0-4.633-2.097-4.633-4.69-.001-2.604 2.073-4.721 4.633-4.721z" />
					<path fill="#FFD43B" transform="translate(0 10.26)" d="M91.682 28.38v10.966c0 8.5-7.208 15.655-15.426 15.655H51.591c-6.756 0-12.346 5.783-12.346 12.549v23.515c0 6.691 5.818 10.628 12.346 12.547 7.816 2.297 15.312 2.713 24.665 0 6.216-1.801 12.346-5.423 12.346-12.547v-9.412H63.938v-3.138h37.012c7.176 0 9.852-5.005 12.348-12.519 2.578-7.735 2.467-15.174 0-25.096-1.774-7.145-5.161-12.521-12.348-12.521h-9.268zM77.809 87.927c2.561 0 4.634 2.097 4.634 4.692 0 2.602-2.074 4.719-4.634 4.719-2.55 0-4.633-2.117-4.633-4.719 0-2.595 2.083-4.692 4.633-4.692z" />
				</>
			)}
			{kind === "typescript" && (
				<>
					<path fill="#fff" d="M22.67 47h99.67v73.67H22.67z" />
					<path fill="#3178C6" d="M1.5 63.91v62.5h125v-125H1.5zm100.73-5a15.56 15.56 0 017.82 4.5 20.58 20.58 0 013 4c0 .16-5.4 3.81-8.69 5.85-.12.08-.6-.44-1.13-1.23a7.09 7.09 0 00-5.87-3.53c-3.79-.26-6.23 1.73-6.21 5a4.58 4.58 0 00.54 2.34c.83 1.73 2.38 2.76 7.24 4.86 8.95 3.85 12.78 6.39 15.16 10 2.66 4 3.25 10.46 1.45 15.24-2 5.2-6.9 8.73-13.83 9.9a38.32 38.32 0 01-9.52-.1 23 23 0 01-12.72-6.63c-1.15-1.27-3.39-4.58-3.25-4.82a9.34 9.34 0 011.15-.73L82 101l3.59-2.08.75 1.11a16.78 16.78 0 004.74 4.54c4 2.1 9.46 1.81 12.16-.62a5.43 5.43 0 00.69-6.92c-1-1.39-3-2.56-8.59-5-6.45-2.78-9.23-4.5-11.77-7.24a16.48 16.48 0 01-3.43-6.25 25 25 0 01-.22-8c1.33-6.23 6-10.58 12.82-11.87a31.66 31.66 0 019.49.26zm-29.34 5.24v5.12H56.66v46.23H45.15V69.26H28.88v-5a49.19 49.19 0 01.12-5.17C29.08 59 39 59 51 59h21.83z" />
				</>
			)}
			{kind === "javascript" && (
				<>
					<path fill="#F0DB4F" d="M1.408 1.408h125.184v125.185H1.408z" />
					<path fill="#323330" d="M116.347 96.736c-.917-5.711-4.641-10.508-15.672-14.981-3.832-1.761-8.104-3.022-9.377-5.926-.452-1.69-.512-2.642-.226-3.665.821-3.32 4.784-4.355 7.925-3.403 2.023.678 3.938 2.237 5.093 4.724 5.402-3.498 5.391-3.475 9.163-5.879-1.381-2.141-2.118-3.129-3.022-4.045-3.249-3.629-7.676-5.498-14.756-5.355l-3.688.477c-3.534.893-6.902 2.748-8.877 5.235-5.926 6.724-4.236 18.492 2.975 23.335 7.104 5.332 17.54 6.545 18.873 11.531 1.297 6.104-4.486 8.08-10.234 7.378-4.236-.881-6.592-3.034-9.139-6.949-4.688 2.713-4.688 2.713-9.508 5.485 1.143 2.499 2.344 3.63 4.26 5.795 9.068 9.198 31.76 8.746 35.83-5.176.165-.478 1.261-3.666.38-8.581zM69.462 58.943H57.753l-.048 30.272c0 6.438.333 12.34-.714 14.149-1.713 3.558-6.152 3.117-8.175 2.427-2.059-1.012-3.106-2.451-4.319-4.485-.333-.584-.583-1.036-.667-1.071l-9.52 5.83c1.583 3.249 3.915 6.069 6.902 7.901 4.462 2.678 10.459 3.499 16.731 2.059 4.082-1.189 7.604-3.652 9.448-7.401 2.666-4.915 2.094-10.864 2.07-17.444.06-10.735.001-21.468.001-32.237z" />
				</>
			)}
			{kind === "markdown" && (
				<>
					<path fill="currentColor" d="M11.95 24.348c-5.836 0-10.618 4.867-10.618 10.681v57.942c0 5.814 4.782 10.681 10.617 10.681h104.102c5.835 0 10.617-4.867 10.617-10.681V35.03c0-5.814-4.783-10.681-10.617-10.681H14.898l-.002-.002H11.95zm-.007 9.543h104.108c.625 0 1.076.423 1.076 1.14v57.94c0 .717-.453 1.14-1.076 1.14H11.949c-.623 0-1.076-.423-1.076-1.14V35.029c0-.715.451-1.135 1.07-1.138z" />
					<path fill="currentColor" d="M20.721 84.1V43.9H32.42l11.697 14.78L55.81 43.9h11.696v40.2H55.81V61.044l-11.694 14.78-11.698-14.78V84.1H20.722zm73.104 0L76.28 64.591h11.697V43.9h11.698v20.69h11.698zm0 0" />
				</>
			)}
		</svg>
	);
}
