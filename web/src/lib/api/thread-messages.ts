 ...
 * Goes through `protocolFetch`, not the axios client: the body is
 * protocol-shaped (snake_case, arbitrary tool payloads) and must not be camelized.
 */
...
export async function fetchThreadMessage(
  ...
  const response = await protocolFetch(path);
  if (!response.ok) {
