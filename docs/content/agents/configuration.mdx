...
- **Tools** — the list discovered at bind time, each one with a status (`always allow`, `needs approval`, `disabled`). See [Tool Settings](/docs/agents/tools).
...
Toggle **Code execution** on the agent page to give the agent a Linux sandbox with shell, file, and search tools. See [Sandbox](/docs/agents/sandbox) for the full toolset and security model.
