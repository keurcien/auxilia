...
Tools are the actions an agent can perform. Most tools come from the MCP servers bound to the agent; a few additional ones come from the [sandbox](/agents/sandbox) when code execution is enabled.
