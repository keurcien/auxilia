...
You don't need to be a developer to follow along, but an admin may need to register MCP servers for you first (see [MCP Servers](/deploy/mcp-servers)).
...
- At least one MCP server should be registered in your workspace. For this example we'll use **HubSpot** (see the [HubSpot setup guide](/deploy/mcp-servers/examples/hubspot)) and a web-search server.

## Step 1 — Create the agent and give it an identity

Navigate to **Agents** in the sidebar and click **New agent**. The editor opens with everything on one page: the identity at the top, the instructions on the left, the tools on the right.
...
Steps 2–4 all happen in this same editor; **Create agent** at the top saves everything at once. Later edits go through **Edit** → **Save changes**.

## Step 2 — Add MCP servers

Tools are what turn a chatbot into an agent. They come from **MCP servers** bound to the agent.

In the tools panel, click **Add tool**. The dialog lists the workspace's MCP servers (and, below them, any sandbox the workspace has configured — see [Using the sandbox](/agents/sandbox)). Add a server with its **+** button; for our example, add two:
...
When you add a server, its card fetches the tool list from the MCP server and shows a **CONNECTED** badge once the tools are in — wait for it before saving, so the agent isn't created with an empty tool map.
...
Your tokens are stored privately and scoped to you — a teammate using the same agent connects with their own account. API-key servers (and bearer-token servers like [n8n](/deploy/mcp-servers/examples/n8n)) skip this step because they share one workspace credential. See [MCP Server Authentication](/deploy/mcp-servers/authentication) for the full picture.
...
Click **Expand** on a bound server card to see its tools. Each tool row carries a three-state toggle:
...
> When you first bind a server, every tool defaults to **Always allow**. Always review the list and tighten writes before sharing the agent — this is the single most important safety step. See [Tools & Permissions](/agents/tools) for the full model.
...
Click **Create agent**. Changes to a saved agent (**Edit** → **Save changes**) take effect on the next message; existing chat threads aren't recomputed. See [Configuration](/agents/configuration) for more prompt-writing tips.

## Step 5 — Test the agent in Chat

Click **Test in chat** on the agent's page (or pick the agent from the **Agents** list) and try a realistic request:
...
- **[Share it with your workspace](/agents/permissions)** — grant teammates or whole teams **member**, **editor**, or **admin** access from the agent's **Permissions** tab
- **[Give it a skill](/agents/skills)** — a reusable procedure from the Skills library, enabled with **Add skill**
- **[Trigger it over HTTP](/tutorials/http-invoke)** — call it from scripts, webhooks, or spreadsheets
- **[Trigger it from Slack](/tutorials/slack)** — let the team talk to it in threads
- **[Run it on a schedule](/agents/triggers)** — daily digests and recurring reports with built-in Triggers
- **[Add subagents](/agents/subagents)** or a **[sandbox](/agents/sandbox)** for more advanced agents
