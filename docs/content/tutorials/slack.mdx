...
**Before you start:** the Slack app must be installed and connected. If it isn't yet, see **[Slack Integration — Setup](/deploy/slack)** (a one-time task for a workspace admin).
...
The agent list the user sees is filtered by their [agent permissions](/agents/permissions) — so members only see agents that have been shared with them.
