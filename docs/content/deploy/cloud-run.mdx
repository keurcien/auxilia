...
**Gemini without an API key**: on Cloud Run, the backend automatically picks up the service account's [Application Default Credentials](https://cloud.google.com/docs/authentication/application-default-credentials). If `GOOGLE_API_KEY` is unset, Gemini models are served through **Vertex AI** using that identity instead — grant the service account the **Vertex AI User** role and you can skip the API key entirely. See [Environment Variables](/deploy/environment-variables#google-via-application-default-credentials).
...
## Optional: agent code execution

If you want agents to run code, Cloud Run is also the easiest place to host the sandbox runtime: deploy the `sandbox-gateway` service, then register it as a **Cloud Run** sandbox under **Settings → Sandboxes** with its URL and secret. See [Cloud Run Sandboxes](/deploy/sandbox/cloud-run). If the sandbox uses a GCS snapshot bucket, the backend's service account needs write access to it.
