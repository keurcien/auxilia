...
This is the inverse of the [n8n & Cloud Scheduler tutorial](/tutorials/n8n-cloud-scheduler), where n8n *calls* auxilia. Here, the agent *calls n8n*.
...
1. Click **Add MCP server** → **Add a custom server**
2. Fill in:
   ...
   See [Authentication methods](/deploy/mcp-servers/authentication) for how the key is encrypted and sent.
3. Save
...
2. Click **Add tool** and pick **n8n Automations**
3. Review the discovered tools and set [tool settings](/agents/tools). Workflows that write to other systems are good candidates for **needs approval** until you trust the agent's judgment.
