...
Because the Redis cache is long-lived (7 days), edits to the file propagate when a workspace admin triggers a sync (`POST /mcp-servers/official/sync`), which force-fetches, validates the file all-or-nothing, and returns the diff.
