...
`SANDBOX_PROVIDER` selects the runtime:

```env
# "opensandbox" (default) or "cloudrun"
SANDBOX_PROVIDER=opensandbox
```

This page covers the **OpenSandbox** provider. For `cloudrun` — Google Cloud Run sandboxes reached through the dedicated sandbox gateway — follow [Cloud Run Sandboxes](/deploy/sandbox/cloud-run) instead; steps 1–3 below don't apply there.
...
If you get `{"enabled": false}`, the selected provider is missing its configuration — `OPEN_SANDBOX_DOMAIN` for `opensandbox`, or `CLOUD_RUN_SANDBOX_GATEWAY_URL` + `CLOUD_RUN_SANDBOX_GATEWAY_SECRET` for `cloudrun`. If the flag is `true` but `create_sandbox` fails at runtime, check the sandbox runtime's logs (the OpenSandbox controller or the sandbox gateway service).
