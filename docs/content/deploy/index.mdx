...
If you only need a quick local run to try the product, head to **[Get Started](/get-started)**. For how to build and configure agents once **auxilia** is up, see **[Build Agents](/agents)**.
...
1. **[Environment variables](/deploy/environment-variables)** — pick a database, configure secrets, choose at least one LLM provider
2. **Deploy the services** — [Cloud Run](/deploy/cloud-run) or [VM / Docker host](/deploy/vm-docker)
3. **[User management](/deploy/user-management)** — create the first admin, invite teammates, create teams, set up Google OAuth, mint personal access tokens
4. **[MCP servers](/deploy/mcp-servers)** — register the tools (Notion, HubSpot, BigQuery, …) you want agents to use
5. **[Sandbox](/deploy/sandbox)** — register a code-execution sandbox (OpenSandbox, Daytona or Cloud Run) under **Settings → Sandboxes** if you want agents to run code
6. **[Observability (Langfuse)](/deploy/langfuse)** — trace LLM calls and tool calls
7. **[Slack integration](/deploy/slack)** — connect your Slack workspace
